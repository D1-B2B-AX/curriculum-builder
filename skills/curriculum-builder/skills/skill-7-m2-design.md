...
| 파일 | 읽는 필드 | 용도 |
|---|---|---|
| `top3-tasks.json` | `per_tool[tool].top3[0]` 전체 | Why 핵심 근거 |
| `task-research/task-cards.json` | 선정된 task의 trigger_text, cadence, action_text | When 업무 맥락 |
| `tool-features.json` | 매칭 feature의 description·useful_for·caveats | 꼭지 2 근거 |
| `m4-{tool}.md` (Skill 5 산출) | "실습 시나리오" + "사용 feature 목록" | M4 연결 |
| `m3-{tool}.md` (Skill 6 산출) | "기초 실습 시나리오" feature 목록 | M3 연결 |
| `input.json` | tools, topic, company, role, matched_topic 참조 | 실행 맥락 |
...
**저장 경로**: `curriculum-builder-output/{run_folder}/m2-{tool_name_safe}.md`
...
본 M2를 아래 제약에 맞춰 재단 (**`input.json` 값 그대로 인용**):
- 시수: {input.json.hours}시간
- 수강생 수준: {input.json.level}
- 보안 제약: {input.json.security}

Skill 9이 위 실제 제약 기반으로 강의 분량·예시 사례·강조점 조정.
```

### 꼭지 설계 가이드 (매우 중요)

**꼭지 수**: 3~5개. task·툴·근거의 풍부함에 따라 조정.
...
5. M3·M4 예고 (수강 흐름 맛보기)

위는 **전형적 예시**이고, task·V3·V4·V2 특성에 따라 꼭지 수·제목이 달라질 수 있음. LLM이 쓰기 나름.
...
### Step 1. 파일 로드
`curriculum-builder-output/{run_folder}/`에서 top3-tasks.json + task-cards.json + tool-features.json + m4-{tool}.md + m3-{tool}.md + input.json 로드.
...
[출력 형식]
SKILL.md §"파일 구조" 템플릿 그대로. 학습 목표 / 배경 근거 / **강의 구성 제안 (꼭지별)** / M3·M4 흐름 / Skill 9 인계 순서.
```
...
---
산출물 파일: curriculum-builder-output/{run_folder}/m2-{tool_name_safe}.md

※ Phase 1 순수 설계입니다. 강의 시수·예시 깊이·발화 수준은 Skill 9(Phase 2)에서 시수·수준에 맞춰 재단됩니다.
...
### Case B (시스템 오류)
- top3-tasks.json / task-cards.json / m4·m3 파일 누락 → 해당 선행 스킬 실행 요구

---

## LD 피드백 처리

- **"OK"** → (다른 세션) Skill 8 (M1 툴 소개) 진행 / 또는 Phase 1 조합
...
- **feature_caveats 대응 설계 금지** — "※ 참고" 형식 표시만 (대응은 Skill 9)
