...
description: curriculum-builder 메인 실습(M4) 설계. Skill 4의 top3-tasks.json에서 top1 task(필요 시 fallback)를 골라 해당 툴로 AX화하는 실습 시나리오를 m4-{tool}.md로 생성. Phase 1 순수 설계 — 보안·시수·수준 제약 미반영 (Skill 9 Phase 2 영역).
---

# Skill 5: M4 메인 실습 설계 (Phase 1 순수)

Skill 4의 top3 중 **top1 task를 해당 툴로 AX화하는 메인 실습**을 설계한다.
...
설계도 §1 Phase 구분:
- **Phase 1 = 목적함수**: "시수·수준·**보안 제약 없이**, 이 직무를 이 툴과 주제로 AX한다면 무엇을 배워야 하는가"의 골자
- **Phase 2 = 제약조건 반영 (Skill 9)**: 시수·수강생 수준·보안 등 실제 운영 제약 재단

**Skill 5는 Phase 1**. 따라서:
- **보안 제약 무시** (`input.json.security` 참조 금지)
- **시수·수준 무시** (`input.json.hours`, `level` 참조 금지)
- 이상적 M4를 설계. Skill 9이 나중에 재단.
...
아래 3개 파일을 읽는다. 모두 `curriculum-builder-output/{run_folder}/` 하위.

| 파일 | 읽는 필드 |
|---|---|
| `top3-tasks.json` | `per_tool` 전체 + 각 top task의 v3/v4/v2·matched_topic·missed_feature_estimates·feature_caveats |
...
- `input.json.security` — Skill 9 영역
- `input.json.hours` — Skill 9 영역
- `input.json.level` — Skill 9 영역
...
**저장 경로**: `curriculum-builder-output/{run_folder}/m4-{tool_name_safe}.md`
...
- 세션 길이 초과·맥락 손실 시의 복구 전략은 Skill 9(Phase 2)에서 시수·수준에 맞춰 보완.
...
- **Skill 9 (Phase 2)**: 본 M4를 아래 제약에 맞춰 재단 (**`input.json` 값 그대로 인용**):
  - 시수: {input.json.hours}시간
  - 수강생 수준: {input.json.level}
  - 보안 제약: {input.json.security}

  Skill 9이 위 실제 제약 기반으로 Step 재구성, 데이터 접근 방식 대체, 분량 조정 등을 수행.
```

### 템플릿 채움 규칙 (매우 중요)

- **"Skill 9 인계" 섹션의 hours/level/security 값은 `input.json`에서 그대로 복사**. 임의 예시·임의 수치 금지 (예: "2시간 단축" 같은 추정 금지).
- input.json에 해당 필드가 null/공백이면 "(미입력)"으로 명시.
- 이 섹션은 LD가 실제 제약을 눈으로 확인하고 Skill 9 실행 결과를 예측하는 용도이므로 정확성 필수.
...
`curriculum-builder-output/{run_folder}/`에서 top3-tasks.json + tool-features.json + input.json 로드.
...
### Step 3. 실습 가능성 판정 + Task 선정

`per_tool[tool].top3` 배열의 **rank 1부터 순회**하며 LLM에 판정 요청.
...
**주의**: "보안 제약으로 이 feature 못 쓴다"는 판정 금지 (Skill 9 영역).
...
2. top1 "불가" → top2 판정
3. top2도 "불가" → top3 판정
4. top3까지 모두 "불가" → Case A 실패 처리 (LD 알림 + 재실행 유도)

**일반적으로 top1이 선정됨.** Killer(K4/K5)로 이미 실습 불가능 task는 걸러졌으므로 top3 진입 task는 기본 실습 가능 전제.
...
[금지]
- feature_caveats 대응 설계 (Skill 9 영역)
- missed_feature_estimates를 시나리오에 포함 (hallucination)
- "보안 제약 때문에 ~"로 시작하는 문장
- 시수 계산 ("N시간 실습")
- "Skill 9 인계" 섹션의 hours/level/security에 임의값 기입 (반드시 input.json 값 그대로)
```
...
---
산출물 파일: curriculum-builder-output/{run_folder}/m4-{tool_name_safe}.md

※ Phase 1 순수 설계입니다. 보안·시수·수준 제약은 Skill 9(Phase 2)에서 재단됩니다.
...
### Case A (LD 조치 가능)
- top3 모두 "불가" 판정 (매우 드묾) → LD에게 알림:
  ```
  ⚠ 이 툴에 대한 M4 설계 불가
  top3 task 모두 이상적 환경에서도 실습 재현 어려움.
  원인: {LLM 판정 사유 요약}
  → Skill 2·4를 다시 실행하거나 task 리스트를 재검토해주세요.
  ```

### Case B (시스템 오류)
- top3-tasks.json·tool-features.json·input.json 누락 → Skill 4 선행 요구 메시지
...
- **"OK" / "진행해"** → Skill 6 (M3 기초 실습)으로 진행
- **"top1 대신 top2로 바꿔줘"** → top2로 M4 재생성 + m4-{tool}.md 덮어쓰기
...
- **feature_caveats 대응 설계 금지** — Skill 4가 LD 인지용으로 ⚠ 표시. Skill 5는 이상적 설계라 무시. 대응은 Skill 9 영역
- **missed_feature_estimates 시나리오 포함 금지** — hallucination 방지. Skill 4가 점수에서 격리한 것을 M4에서 되살리면 안 됨
- **Killer 탈락 task 사용 금지** — top3에만 집중
- **임의 task 교체 금지** — LD 요청 시만
- **시간 표기 금지** — "2시간 실습" 등 시수 추정 시도 금지. Phase 2 영역
- **"파일 업로드 차단 시" 등 조건부 대응 서술 금지** — Skill 9가 할 일
