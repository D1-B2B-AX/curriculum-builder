---
name: top-task-scorer
description: curriculum-builder 실습 task 후보 선정. Skill 2(task 리스트 + DNA + workflow)와 Skill 3(tool-features) 결과를 받아 V4(툴 적합성)/V3(AI 변화 폭)/V2(직무 임팩트) 3축으로 점수화, Killer Criteria 필터 후 task 후보를 top-tasks.json에 저장. LD가 직접 선택할 수 있도록 후보 영역 (top + 잠재 후보) 박음. Skill 5~7이 M4~M2 설계에 이 결과를 참조.
---

# Skill 4: 실습 task 후보 선정

Skill 2가 도출한 직무 task 목록 중 **메인 실습(M4)에 적합한 task 후보**를 선정한다 (top + 잠재 후보 영역).
V4(툴 적합성), V3(AI 변화 폭), V2(직무 임팩트) 3축으로 평가하고, Killer Criteria로 사전 필터 후 가중합 점수로 상위 3개를 뽑는다.

## 왜 이 스킬이 필요한가

Skill 2가 도출한 task 목록 중 "실습으로 가장 가치 있는 후보"를 **객관적·반복 가능한 기준**으로 고르기 위함. 기준 없이 고르면 매번 다른 결과가 나온다. Skill 5~7은 이 후보와 평가 근거를 M4·M3·M2 설계의 **교육적 설명 소스**로 활용한다.
...
아래 6개 파일을 읽는다. 모두 Skill 1·2·3이 생성한 `{run_folder}/` 하위에 있다.
...
| `phase1/task-research/task-cards.json` | trigger, inputs, action, immediate_output, cadence | K3 체크, V2 빈도, LLM 평가 입력 |
| `phase1/task-research/task-dna.json` | primitive_lv1/2, domain_lv1/2, mechanism_lv1/2, classification_status | K1/K2 체크 |
| `phase1/task-research/workflow.json` | DAG edges | V2 중심성 |

---

## Output: top-tasks.json

**저장 경로**: `{run_folder}/top-tasks.json` (단일 파일)
...
top과 동일하되 `rank`가 4~6 사이, `high_potential_low_match`는 반드시 `true`.
...
상위 task를 `top`에 저장. Killer 통과 task가 부족하면 그대로 반환 (경고는 Step 4에서 이미 표시).

### Step 7. 고잠재력 외부 노출 + 다양성 체크 + matched_topic + 저장

#### 7-1. 잠재 후보 (4~6위) + top 5 분기

4위부터 6위까지 스캔, `v3.score ≥ 5 && v4.primary_score ≤ 1`인 task 추출.

**분기**:
- 추출 결과 **2개 이상** → top 3 + potential 2 = 5건 (`potential`에 상위 2개 기록·`high_potential_low_match = true`)
- 추출 결과 **0~1개** → 잠재 후보 영역 폐기·top 5 (4~5위 점수 기준 task로 채움·`potential` 빈 배열)

총 후보 5건이 LD 검토 영역으로 박힘.

#### 7-2. matched_topic 태깅 (복수 주제일 때만)
`topic_input.is_composite = true`인 경우, `references/prompts_meta.md`의 matched_topic 태깅 프롬프트로 각 top·potential task에 `matched_topic` 필드 채움.

#### 7-3. 다양성 체크 (복수 주제일 때만)
`references/prompts_meta.md`의 다양성 체크 프롬프트로 top의 matched_topic 분포 판정 → `diversity_check` 블록 기록.

#### 7-4. JSON 저장
최종 구조를 `{run_folder}/top-tasks.json`에 저장.

#### 7-5. LD 자연어 요약 생성 + LD 직접 선택 분기 (★ 4/21 회의 결정)

`references/prompts_meta.md`의 LD 요약 생성 프롬프트로 top + potential을 **자연어 문장**으로 변환 출력.
- rubric 라벨 대신 **의미 문장** 사용 (rubric.md §LD 변환 규칙 참조)
- Killer 탈락은 LD에 미노출 (남은 task 3개 미만 경고만 예외)

**LD 직접 선택 분기**: LD 요약 출력 후 LD가 5개 후보 중 메인 실습 (M4) 설계할 task 직접 선택.
- LD 답변 받음 → 선택 task → Skill 5·6·7 (M4·M3·M2 설계) 진입
- LD 답변 X (또는 "기본"·"진행해" 등 모호 답변) → **top 1 fallback** (자동 선정)
- 선택 task가 potential 영역이면 high_potential_low_match 영역에서 자연 진입 (skill-5가 LD 선택 task 받음)
...
[분기 — top 3 + potential 2 영역이면]
후보 4 [Potential]: ...
  ⚠ AI 전환 가치는 매우 크지만 툴 매칭이 약해 잠재 후보 영역. 재설계로 실습 가능할지 검토 권장.
후보 5 [Potential]: ...
...
[복수 주제 + 쏠림 시]
  ⚠ 후보 5개가 '{skewed_to}' 주제로 쏠려 있습니다.
     다른 주제({주제 목록}) 반영을 위해 재실행하시겠습니까?

★ 위 5개 후보 중 어느 task로 메인 실습 (M4) 설계할지 직접 선택해주세요.
   후보 번호를 선택하거나 task를 직접 입력하시면 됩니다.
...
- **"top1을 potential의 X로 바꿔줘"** → `top` 배열에서 교체 + top-tasks.json 재저장 (final_score 재계산 없이 rank만 조정)
...
- **임의로 top 후보를 편집하지 않는다.** LD 요청 시에만 수정.
