...
Skill 4(실습 task top3 선정)에서 스코어링 대상이 되는 task 풀을 만드는 단계다.
"이 직무의 실제 task → DNA 분류 → 카드화 → 워크플로우"까지 나와야 Skill 4가 근거 있는 선택을 할 수 있다.

## Input

- 가장 최근 생성된 실행 폴더(`curriculum-builder-output/{company}_{role}_{YYYYMMDDHHMM}/`) 안의 `input.json`에서 **`company`, `role`, `run_folder`를 읽는다.**
- `run_folder`는 Skill 1이 기록한 실행 폴더 이름(상대 경로). Skill 2의 모든 산출물은 이 폴더 안에 저장한다.
...
모든 산출물은 **실행 폴더**(`curriculum-builder-output/{run_folder}/`) 내부의 `task-research/` 하위에 저장한다. `{run_folder}`는 Skill 1이 생성한 폴더 이름(input.json의 `run_folder` 필드).

| 단계 | 저장 경로 | 내용 |
|------|----------|------|
| 2-1 | `curriculum-builder-output/{run_folder}/task-research/raw-tasks.json` | company-role-task-research의 output (tasks + sources) |
| 2-2 | `curriculum-builder-output/{run_folder}/task-research/atomic-tasks.json` | task-atomization의 output (atomic tasks 배열) |
| 2-3 | `curriculum-builder-output/{run_folder}/task-research/task-dna.json` | task-dna-classification의 output (DNA가 붙은 tasks) |
| 2-4 | `curriculum-builder-output/{run_folder}/task-research/task-cards.json` | task-card-generation의 output (task cards 배열) |
| 2-5 | `curriculum-builder-output/{run_folder}/task-research/workflow.json` | workflow-reconstruction의 output (DAG) |
...
**저장 경로**: `curriculum-builder-output/{run_folder}/tasks.json`

```json
{
  "company": "현대카드",
  "role": "마케팅팀 퍼포먼스 마케터",
  "raw_tasks_ref": "task-research/raw-tasks.json",
  "atomic_tasks_ref": "task-research/atomic-tasks.json",
  "task_dna_ref": "task-research/task-dna.json",
  "task_cards_ref": "task-research/task-cards.json",
  "workflow_ref": "task-research/workflow.json",
  "summary": {
    "n_tasks": 26,
    "n_streams": 3
  }
...
1. `input.json`을 읽어 `company`, `role`, `run_folder`를 확인한다. 이후 모든 저장 경로는 `curriculum-builder-output/{run_folder}/` 기준.
...
10. 최종 요약을 LD에게 보고한다. `task-cards.json`의 `one_liner` 필드를 활용해 업무 목록을 나열한다:

```
✅ 직무 분석 완료

{회사} {직무}의 주요 업무 {M}개를 파악했습니다.
...
💡 업무 간 연결 흐름 {S}개도 함께 정리되어 있어, 다음 단계에서 참고됩니다.

산출물: curriculum-builder-output/tasks.json

위 업무 목록을 확인해주세요. 이상하거나 빠진 업무가 있으면 알려주시고, 문제없으면 이대로 다음 단계(툴 기능 조사)로 넘어가겠습니다.
```

`atomic_task`가 3~4개인 경계 케이스에서는 위 요약 본문 아래에 한 줄을 덧붙인다:
> "※ 파악된 업무가 {M}개로 다소 적은 편입니다. 다음 단계에서 선택지가 제한될 수 있습니다."
...
최종 요약 후 LD의 답변에 따라 다음과 같이 대응한다:

- **"진행해" / "OK" / 답변 없음** → Skill 3(툴 기능 조사)으로 진행
- **특정 업무가 빠짐/이상하다** → 관련 단계부터 재실행 (보통 2-1 원본 task 도출 단계부터)
- **전반적 방향이 다르다** → 전체 파이프라인 재실행
- **업무를 더 많이 뽑아달라** → 2-1의 검색 전략 조정 후 전체 재실행

Skill 2가 **임의로 결과를 수정하지 않는다.** LD의 의도를 확인하고 재실행으로 대응한다.
