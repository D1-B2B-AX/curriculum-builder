...
3. **비교 및 분기** (SHA 표기 시 **앞 7자**로 축약)
   - **동일** → 아무 안내 없이 STEP 1 진행
   - **다름** (또는 로컬 미설치) → `~/.curriculum-builder/install_path.txt` 존재 여부 확인 후 아래 둘 중 하나로 분기:

     **3-1. 자동 업데이트 흐름** (install_path.txt 존재 시 — 기본 경로)
     ...
     - LD "예" / "응" / "진행" → install_path.txt 내용을 읽어 `{install_path}` 추출 후 PowerShell로 자동 실행 (`-ExecutionPolicy Bypass`로 PowerShell 보안 정책 차단 회피):
       ...
       ```
       [업데이트 완료]
       업데이트가 적용되었습니다. 새 Claude Code 세션에서 /curriculum-builder 를 다시 실행해주세요.
       ```
       본 세션 파이프라인 종료 (재로드 위해 새 세션 필요).

     - LD "아니오" / "skip" / "이대로" → STEP 1로 진행 (기존 버전 그대로)
     - 무응답 / 모호 → "예/아니오" 한 번 더 확인 후 결정. 그래도 모호하면 "이대로 진행"으로 처리

     **3-2. 수동 안내 흐름** (install_path.txt 부재 시 — fallback. install.ps1 패치 도입 전 install한 LD용)
...
- LD가 "이대로 진행"을 선택하면 버전 차이를 이유로 기능을 제한하지 않는다 — 기존 로컬 스킬로 정상 실행
- SHA 전체(40자)를 LD 출력에 그대로 노출하지 않는다. 가독성을 위해 앞 7자만 표시
- install_path.txt 기록·읽기 실패 시 3-2 수동 안내 흐름으로 자동 fallback. spec 동작은 어느 환경에서도 보장
