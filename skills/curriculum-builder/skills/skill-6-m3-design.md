...
description: curriculum-builder 기초 실습(M3) 설계. Skill 5가 만든 m4-{tool}.md를 읽어 M4에서 쓰는 feature 각각에 대한 기초 사용법 mini 실습을 m3-{tool}.md로 생성. Phase 1 순수 설계 — 보안·시수·수준 제약 미반영 (Phase 3 영역).
---
...
- feature_caveats 무시 (Skill 4가 LD 인지용으로 표시. Phase 3 영역)
...
아래 3개를 읽는다. 모두 `{run_folder}/` 하위.

| 파일 | 읽는 필드 |
|---|---|
| `phase1/m4-{tool}.md` (Skill 5 산출) | **"사용 feature 목록"** 섹션 — 각 feature_name + 활용 방식. **"실습 시나리오"** 섹션 — M4 맥락 파악용 참조 |
| `tool-features.json` (`{run_folder}/` 직속) | 각 feature의 name / description / useful_for (기초 실습 시나리오 작성 시 활용) |
| `input.json` (`{run_folder}/` 직속) | `tools[]`, `topic`, `company`, `role` — **security·hours·level 미사용** |

### 사용하지 않는 필드

- `input.json.security/hours/level` — Phase 3 영역
- `tool-features.json`의 feature `caveats` — Phase 3 영역
- `top-tasks.json`의 `missed_feature_estimates` — hallucination 격리 유지

---

## Output: m3-{tool}.md

**저장 경로**: `{run_folder}/phase1/m3-{tool_name_safe}.md`
...
`{run_folder}/phase1/`에서 `m4-{tool}.md`, `{run_folder}/` 직속에서 `tool-features.json`·`input.json` 로드.
...
3. mini 입력은 M4보다 **훨씬 작은 규모**임을 명시하되, **구체 수치(N행/N건/N턴 등) 기입 금지**. "작은 샘플", "짧은 예시", "몇 건", "몇 턴의 연속 대화" 등 추상 표현으로. 구체 규모는 Phase 3 자유 대화에서 수강생 수준·시수에 맞춰 정함.
...
---
산출물 파일: {run_folder}/phase1/m3-{tool_name_safe}.md

※ Phase 1 순수 설계입니다. 분량·난이도는 Phase 3 자유 대화에서 시수·수준에 맞춰 재단됩니다.
...
**기본 = 자동 진행** — Skill 6은 LD 출력 후 응답을 끊지 않고 Skill 7로 바로 이어간다 (STEP 5~8은 한 턴 안에서 흐르는 구간이라 LD 응답을 받는 지점이 아님 — LD가 끼어들 수 있는 건 다음 주요 확인 포인트인 Phase 1 조합 끝). 만일 LD가 능동적으로 개입하면 아래대로 반영:

- **"OK" / "진행해" / "계속"** → Skill 7 (M2 When/Why)으로 진행
...
- **구체 시간 수치 기입 금지** — "5~10분" 같은 표현 대신 "짧은 mini 실습" 등 분량 추상 표현 사용. 시수는 Phase 3 영역
- **구체 입력 수치 기입 금지** — "10~20행", "5~7건", "4턴" 등 특정 수치 사용 금지. "작은 샘플", "짧은 예시", "몇 건", "몇 턴의 연속 대화" 등 추상 표현으로. 구체 규모는 Phase 3 자유 대화에서 수강생 수준·시수에 맞춰 정함
- **feature_caveats 대응 설계 금지** — Phase 3 영역
